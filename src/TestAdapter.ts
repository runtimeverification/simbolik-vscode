...
import { testFiles, forgeTest, forgeCoverage, ForgeTestOptions, ForgeTestSuiteReport } from './foundry';
...
export async function createTestController() : Promise<vscode.TestController> {
    ...
    const debugProfile = testController.createRunProfile(
        ...
        async (request, token) => {
            for (const item of request.include ?? []) {
                const { workspaceFolder, file, contract, method } : TestContext = testTree.items.get(item) ?? {};
                if (!contract || !method) {
    ...
    const runProfile = createFoundryProfile(testController, testTree, vscode.TestRunProfileKind.Run);
...
function testContextToTestOptions(context: TestContext) : ForgeTestOptions {
    ...
    if (context.file) {
        options.path = vscode.workspace.asRelativePath(
            context.file,
            false
        );
    }
...
class IndexedTestTree {
    ...
    matchLeafs(context: TestContext) : Map<vscode.TestItem, Required<TestContext>> {
        const results = new Map<vscode.TestItem, Required<TestContext>>();
        for (const [item, ctx] of this.items) {
            if (ctx.method === undefined) {
                continue;
            }
            if (context.workspaceFolder && context.workspaceFolder !== ctx.workspaceFolder) {
                continue;
            }
...
function createFoundryProfile(
    ...
    return testController.createRunProfile(
        profileKind === vscode.TestRunProfileKind.Run ? 'Run Foundry Tests' : 'Run Foundry Tests with Coverage',
        profileKind,
        async (request, token) => {
            ...
            for (const item of request.include ?? []) {
                ...
                if (!workspaceFolder) {
                    run.errored(
                        item,
                        new vscode.TestMessage('Cannot determine workspace folder for test item.')
                    );
                    continue;
                }
                ...
                try {
                    ...
                    processCoverageReport(lcovRecords, workspaceFolder, run, coverageCache);
...
function processTestReport(
    ...
    for (const [item, context] of leafs) {
        ...
        const testResult = Object.entries(reportEntry?.test_results).find(([id, tr]) => id.startsWith(method.name + '('));
        if (!testResult) {
            const message = new vscode.TestMessage(
...
function processCoverageReport(
    ...
    workspaceFolder: vscode.WorkspaceFolder,
    run: vscode.TestRun,
    coverageCache: WeakMap<vscode.FileCoverage, vscode.StatementCoverage[]> | undefined
) {
    for (const lcovRecord of lcovRecords) {
        ...
        const fileUri = vscode.Uri.joinPath(workspaceFolder.uri, ...lcovRecord.file.split('/'));
...
async function createTestTree(testController: vscode.TestController) : Promise<IndexedTestTree> {
    ...
    for (const workspaceFolder of workspaceFolders) {
        ...
        for (const file of files) {
            const fileItem = await createFileTestItem(
                testController,
                file
            );
                ...
                ContractDefinition: async contract => {
                    ...
                    parser.visit(contract, {
                        FunctionDefinition: async func => {
                            ...
                            if (methodItem) {
                                testTree.items.set(methodItem, { workspaceFolder, file, contract, method: func });
...
async function createContractTestItem(
    ...
    if (contract.loc) {
        contractItem.range = new vscode.Range(
            new vscode.Position(contract.loc.start.line - 1, contract.loc.start.column),
            new vscode.Position(contract.loc.start.line - 1, contract.loc.start.column)
        );
    }
    return contractItem;
