...
async function downloadAndExtract(url: string): Promise<void> {
  ...
  for (const entry of files) {
    ...
    if (entry.typeflag === FILETYPE_DIRECTORY) { // '5' indicates a directory in tar format
      await vscode.workspace.fs.createDirectory(uri);
    } else if (entry.typeflag === FILETYPE_FILE || entry.typeflag === '') { // '0' or '' indicates a file in tar format
